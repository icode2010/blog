...
start_rule() {
    ...
    local cmd="$SOCAT_CMD ${proto_upper}-LISTEN:${port},fork,reuseaddr ${proto_upper}:[${target_ip}]:${target_port}"
    nohup $cmd > /dev/null 2>&1 &
    local pid=$!
    echo "$pid" > "$pid_file"
    echo "[已启动] 规则 ${rule_id} -> ${target_ip}:${target_port} (PID: $pid)"
}

stop_rule() {
    local rule_id=$1
    local pid_file="$PID_DIR/${rule_id}.pid"
    if [ ! -f "$pid_file" ]; then echo "[已停止] 规则 ${rule_id} (未找到PID文件)"; return; fi
    local pid; pid=$(cat "$pid_file")
    if ps -p "$pid" > /dev/null; then kill "$pid"; echo "[已停止] 规则 ${rule_id} (PID: $pid)";
    else echo "[已失效] 规则 ${rule_id} 的进程 (PID: $pid) 已不存在, 清理PID文件。"; fi
    rm -f "$pid_file"
}
...
list_rule() {
    ...
    local pid_file="$PID_DIR/${rule_id}.pid"
    if [ -f "$pid_file" ] && ps -p "$(cat "$pid_file")" > /dev/null; then
        echo -e "\e[32m[运行中]\e[0m ${proto}/IPv6 端口 ${port} -> [${target_ip}]:${target_port} (PID: $(cat "$pid_file"))"
    else
        echo -e "\e[31m[已停止]\e[0m ${proto}/IPv6 端口 ${port} -> [${target_ip}]:${target_port}"
    fi
}

# --- 主逻辑 ---
case "$1" in
    start) echo "--- 正在启动所有转发规则 ---"; process_rules start_rule; echo "---"; ;;
    stop) echo "--- 正在停止所有转发进程 ---"; for pf in "$PID_DIR"/*.pid; do [ -e "$pf" ] || continue; stop_rule "$(basename "$pf" .pid)"; done; echo "---"; ;;
    status|list) echo "--- 当前转发规则状态 ---"; process_rules list_rule; echo "---"; ;;
    restart) $0 stop; sleep 1; $0 start; ;;
    *) echo "用法: sudo $(basename "$0") {start|stop|restart|status|list}"; exit 1; ;;
esac
exit 0
```
